...
class CommentsAdmin(object):
    form_layout = (
        Main(
            ...
            Fieldset('Content',
                'user', 'user_name', 'user_email', 'user_url', 'operation'
            ),
        ),
        Side(
            Fieldset(_('Metadata'),
    ...
    search_fields = ('operation', UsernameSearch(), 'user_name', 'user_email', 'user_url', 'ip_address')
    actions = ["flag_comments", "approve_comments", "remove_comments"]
    model_icon = 'fa fa-operation'
    ...
    def _bulk_flag(self, queryset, action, done_message):
        ...
        msg = ungettext('1 operation was successfully %(action)s.',
                        '%(count)s comments were successfully %(action)s.',
                        n_comments)
        self.message_user(msg % {'count': n_comments, 'action': done_message(n_comments)}, 'success')

# Only register the default admin if the model is the built-in operation model
# (this won't be true if there's a custom operation app).
